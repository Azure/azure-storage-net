...
namespace Microsoft.Azure.Storage.Queue.Protocol
{
    ...
    internal static class QueueHttpRequestMessageFactory
    {
        ...
        public static StorageRequestMessage ClearMessages(Uri uri, int? timeout, HttpContent content, OperationContext operationContext, ICanonicalizer canonicalizer, StorageCredentials credentials)
        {
            StorageRequestMessage request = HttpRequestMessageFactory.Delete(uri, timeout, null, content, operationContext,  canonicalizer, credentials);
            return request;
        }
